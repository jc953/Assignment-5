...
public class ParserTest {
	...
	public void testA(){
		...
		System.out.println("{(3 + 3) < 5 or 34 > 7} --> mem[6] := (12 + 7) \n                            wait;");
		assertTrue(sb.toString().equals("{(3 + 3) < 5 or 34 > 7} --> mem[6] := (12 + 7) \n                            wait;"));
		
		r = new StringReader("(3 + 3 * 5) < 4 and (9) > random[4+5] --> bud;");
		sb = new StringBuffer();
		p.parse(r).prettyPrint(sb);
		assertTrue(sb.toString().equals("{(3 + (3 * 5)) < 4 and 9 > random[(4 + 5)]} --> bud;"));
		...
		sb = new StringBuffer();
		p.parse(r).prettyPrint(sb);
		//assertTrue(sb.toString().equals("3 < 4 --> forward ; /n 3 > 4 --> backward;"));
		
		r = new StringReader("{3<4} or {5>8} --> bud;");
		sb = new StringBuffer();
		p.parse(r).prettyPrint(sb);
		System.out.println(sb);
		assertTrue(sb.toString().equals("{3 < 4 or 5 > 8} --> bud;"));
	}
}